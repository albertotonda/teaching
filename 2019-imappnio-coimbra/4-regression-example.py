...
cross_validation_results = cross_validate(model, X, y, scoring='r2', cv=n_folds, return_estimator=True, return_train_score=True) # return_estimator puts all trained models in the dictionary, under the key "estimator"

# let's see how we performed
import numpy as np
print("Average R2 score (on training) for model %s is %.4f (+/- %.4f)" % (model.__class__.__name__, np.mean(cross_validation_results["train_score"]), np.std(cross_validation_results["train_score"])))
